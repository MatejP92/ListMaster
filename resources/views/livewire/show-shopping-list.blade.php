        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($shoppingList as $item)
                    <tr class="{{ $item->finished ? 'bg-green-200 dark:bg-green-800' : 'odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800' }} border-b dark:border-gray-700">
                        ...
                        <td class="pl-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                            @if($item->finished)
                                <button wire:click="unfinished({{ $item }})" class="font-medium text-green-600 dark:text-yellow-500 hover:underline pr-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
                                    </svg>
                                </button>
                            @else
                                <button wire:click="finished({{ $item }})" class="font-medium text-green-600 dark:text-green-500 hover:underline pr-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                            ...
                            <button wire:click='deleteItem({{ $item }})' wire:confirm="Delete {{ $item->item }}?" class="font-medium text-red-600 dark:text-red-500 hover:underline">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
